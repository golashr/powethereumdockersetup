...
if [ "$CHAIN_TYPE" == "private" ]; then
  ...
  ls -la $DATA_DIR
  if ([ ! -d "$DATA_DIR" ] || [ -d "ls -A $DATA_DIR" ]) && [ "$RUN_BOOTNODE" != "true" ]; then



      echo "Creatng new account before Initializing geth!"
      geth --datadir "$DATA_DIR" --password /opt/pwd.txt account new > .ACCOUNT
	    GEN_ALLOC=$(head -n 1 .ACCOUNT)
	    echo GEN_ALLOC is $GEN_ALLOC
      echo "Generating account number from geth..."
      sed "s/\${GEN_ALLOC}/$GEN_ALLOC/g" -i /opt/genesis.json
